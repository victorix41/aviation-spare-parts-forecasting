...
def normalise_criticality(value: object) -> str:
...
def calculate_service_level_z_score(
    ...
    """Convert a service level into its normal-distribution z-score."""

    if not 0.50 < service_level < 1.0:
        raise InventoryOptimisationError(
            "Service level must be greater than 0.50 and below 1.00."
        )

    return float(
        NormalDist().inv_cdf(service_level)
    )
...
def calculate_safety_stock(
    *,
    ...
    return max(
        ...
        z_score
        * demand_standard_deviation
        * math.sqrt(max(lead_time_months, 0.0)),
    )
...
def create_recommendation_reason(
    *,
    ...
    if recommended_order_quantity <= 0:
        return (
            f"No immediate order is indicated. Current balance "
            f"({current_balance:.2f}) meets or exceeds the calculated "
            f"target stock ({target_stock:.2f})."
        )

    return (
        f"{stockout_risk} stock risk. Current balance "
        f"({current_balance:.2f}) is below the calculated inventory "
        f"requirement. Reorder point: {reorder_point:.2f}; "
        f"target stock: {target_stock:.2f}; average lead time: "
        f"{lead_time_days:.1f} days. Recommended order quantity: "
        f"{recommended_order_quantity:.0f} units, subject to authorised "
        f"human review."
    )
...
def prepare_inventory_summary(
    ...
    *,
    ...
    output["balance_quantity"] = pd.to_numeric(
    ...
    if "purchase_order_date" not in output.columns:
        output["purchase_order_date"] = pd.NaT

    if "delivery_order_date" not in output.columns:
        output["delivery_order_date"] = pd.NaT
    ...
    output["lead_time_days"] = output.apply(
        lambda row: calculate_lead_time_days(
            ...
            minimum_lead_time_days=minimum_lead_time_days,
        ),
        axis=1,
    )

    output["inventory_line_value_usd"] = (
        output["balance_quantity"]
        * output["unit_price_usd"]
    )
...
def run_inventory_optimisation(
    *,
    ...
    minimum_lead_time_days = float(
        settings["minimum_lead_time_days"]
    )
